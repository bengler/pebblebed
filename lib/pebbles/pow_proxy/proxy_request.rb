 # Annexed from https://github.com/aniero/rack-streaming-proxy
  ...
  class ProxyRequest
    ...
    def initialize(request, uri)
      ...
      proxy_request["X-Forwarded-For"] =
        (request.env["X-Forwarded-For"].to_s.split(/\s*,\s*/) + [request.env["REMOTE_ADDR"]]).join(", ")
      proxy_request["X-Forwarded-Host"] = 
        (request.env["X-Forwarded-Host"].to_s.split(/\s*,\s*/) + [request.env["HTTP_HOST"]]).join(", ")
